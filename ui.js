$(async function () {
  ...
  $navLogOut.on('click', async function () {
    localStorage.clear()
    $('.articles-container').slideUp(600)
  ...
  function fillUserProfile () {
    ...
    $('#profile-account-date').text(
      `Account Created: ${localStorage.getItem('createdAt')}`
    )
  }
  fillUserProfile()
  ...
  function favPlaceHolder () {
    if (
      ...
      $('#favorited-articles').append(
        '<li id="fav-placeholder">No favorites added</li>'
      )
    } else {
  ...
  async function favAppendOrRemove (event) {
    ...
    if (heartSpan.classList.contains('fav')) {
      const res = await StoryList.addFavorite(
        localStorage.username,
        heartSpan.parentElement.id
      )
    } else {
      const res = await StoryList.deleteFavorite(
        localStorage.username,
        heartSpan.parentElement.id
      )
    }
